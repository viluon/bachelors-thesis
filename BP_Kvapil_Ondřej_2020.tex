...
\begin{listing}[h]
	...
	\begin{minted}[autogobble]{haskell}
		...
		snd (x, y) = y
	\end{minted}
	\caption{The right projection of an ordered pair of integers, non-strict in
...
\begin{listing}[h]
	...
	\end{minted}
	\caption{Non-strict semantics avoids unnecessary computations.}
	\label{lst:foo}
\end{listing}
...
\begin{listing}[h]
	...
	\end{minted}
	\label{lst:foo'}
	\caption{Non-strictness can avoid runtime errors.}
\end{listing}
...
\begin{listing}[h]
	...
	\begin{minted}[autogobble]{haskell}
		...
		in  x `par` f x
	\end{minted}
	\caption{\acrshort{ghc} shares delayed computations and their results, even
...
which does not involve optimisations, the conversion to \acrshort{stg}, or any
later passes. Instead, the separate bytecode generator translates Core directly
to bytecode instructions for execution by the \acrshort{rts} interpreter,
