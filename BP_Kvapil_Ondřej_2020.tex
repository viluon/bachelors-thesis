...
understanding, we \todo{how do I refer to myself?} design a dynamic tracing
tool capable of capturing information about the runtime behaviour of non-strict
functions.

\subsection{The \acrshort{ghci} approach}
...
The Haskell programmer invokes \acrshort{ghci} either via its dedicated
executable or by passing the \texttt{--interactive} command-line flag to
\acrshort{ghc}. The user's expression entered at the prompt is fed through a
...
The second approach to dynamic tracing, which we\todo{?} eventually settled on,
involves a relatively recently introduced feature of \acrshort{ghc}: source
plugins\cite{ghc-source-plugins}. Compiler source plugins are Haskell packages
which invoke the \acrshort{ghc} API\todo{same as UI, I suppose?} to hook into
the compiler pipeline and modify the compiled program at various stages of the
front-end. Unlike Core plugins, which operate on the internal language, source
plugins deal with the entirety of Haskell's surface syntax.
...
strictness of a function on a case-by-case basis. While retaining the order of
...
case with the previous approach.
