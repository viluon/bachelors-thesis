...
\todo{should have a custom section, I suppose}
The \acrfull{ghci} is an interactive interface built on \acrshort{ghc}'s
bytecode compilation pipeline and the bytecode interpreter of the
\acrshort{rts}. \acrshort{ghci} offers a read-eval-print loop popular in other
programming languages. A notable feature of the tool is the \acrshort{ghci}
debugger, which allows the programmer to place breakpoints on certain
expressions in their code. The interpreter then pauses execution when it is
about to evaluate an expression marked by a breakpoint.
...
\todo{should we describe the bytecode format?}

Internally, breakpoints rely on a special bytecode instruction.

While its command-line interface is a separate module, the
support for various \acrshort{ghc} features intended predominantly for the 
